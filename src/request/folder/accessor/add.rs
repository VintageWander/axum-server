...
pub struct FolderAddAccessorRequest {
    #[validate(email(message = "Invalid email"))]
    pub email: String,
}

#[async_trait]
impl FromRequest<Service, Body> for FolderAddAccessorRequest {
    type Rejection = Error;
    async fn from_request(req: Request<Body>, state: &Service) -> Result<Self, Self::Rejection> {
        let Json(body) = Json::<FolderAddAccessorRequest>::from_request(req, state).await?;
