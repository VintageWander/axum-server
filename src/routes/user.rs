...
use crate::{
    handler::{
        auth::logout::logout_user_handler,
        user::{
...
pub fn user_routes() -> Router<SharedState> {
    Router::new().nest(
        ...
        Router::new()
            ...
            .merge(login_user_route())
            .merge(profile_user_route())
            .merge(logout_user_route())
...
pub fn login_user_route() -> Router<SharedState> {
    Router::new().route("/login", post(profile_user_handler))
}
...
pub fn logout_user_route() -> Router<SharedState> {
    Router::new().route("/logout", delete(logout_user_handler))
}
